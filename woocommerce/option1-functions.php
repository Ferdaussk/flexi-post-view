...
echo '<div class="ferdaussk_for_unique '.$pa_presets_cls.'">';
    ...
    <div id="icon_preview" style="font-size: 24px; margin-top: 20px;">
        Icon Preview: <i class="<?php echo esc_attr(get_option('flpvi-breadcrumb-icon-control', 'fa fa-adjust')); ?>" id="selected_icon_preview"></i>
    </div>
    ...
    $selectedFont = get_option('flpvi-fontfamilly', 'Arial, sans-serif');
    ...
    ?>
                ...
                <?php
                if ($flpvi_sale_check_value != false) { // for testing font familly and icon controls
                    if ($post->is_on_sale()) {
                ...
                <article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
                    ...
                    <div class="ferdaussk-name">
                        <div class="summary entry-summary">
                            ...
                            if ($post->is_on_sale()) {
                                ...
                                echo '<p class="price">' . wc_price($post->get_price()) . '</p>';
                            }
                            ?>
                            ...
                            <form class="cart" method="post" enctype='multipart/form-data'>
                                ...
                                <button type="submit" name="add-to-cart" value="<?php echo esc_attr($post->get_id()); ?>" class="single_add_to_cart_button button alt"><?php echo esc_html($post->single_add_to_cart_text()); ?></button>
                                <?php
                    ...
                    $tabs = array(
                        ...
                        'reviews'     => array(
                            'title'    => sprintf( __( 'Reviews (%d)', 'woocommerce' ), $post->get_review_count() ),
                            'priority' => 30,
                    ...
                    <div class="related-posts">
                        ...
                        // For the exclude related post start
                        $related_product_exclude = [1,2,3,4]; // here should to add the post id
                        $related_ids = array_diff($related_ids, $related_product_exclude);
                        // For the exclude related post end
                        // Display related posts
                        if (!empty($related_ids)) {
                            echo '<h2>' . esc_html__('Related Posts', 'flexi-post-view') . '</h2>';
                            ...
                            echo '<p>' . esc_html__('No related posts found.', 'flexi-post-view') . '</p>'; // The messege should not show
                        }
                        ?>
                    </div>
                    <!-- related-posts -->
